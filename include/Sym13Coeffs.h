...
static const std::array<double, length> dec_lo = {
...
static const std::array<double, length> dec_hi = []{
  std::array<double, length> a{};
  for(size_t i=0;i<length;++i)
    a[i] = ((i&1)? 1.0:-1.0) * dec_lo[length-1-i];
  return a;
}();

// 重构低通 rec_lo 直接是 dec_lo 逆序
static const std::array<double, length> rec_lo = []{
  std::array<double, length> a{};
  for(size_t i=0;i<length;++i)
    a[i] = dec_lo[length-1-i];
  return a;
}();

// 重构高通 rec_hi 直接是 dec_hi 逆序
static const std::array<double, length> rec_hi = []{
  std::array<double, length> a{};
  for(size_t i=0;i<length;++i)
    a[i] = dec_hi[length-1-i];
  return a;
}();
