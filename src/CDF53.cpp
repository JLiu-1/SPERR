...
template auto sperr::CDF53::copy_data(const float*, size_t, dims_type) -> RTNType;
template auto sperr::CDF53::copy_data(const double*, size_t, dims_type) -> RTNType;

auto sperr::CDF53::take_data(vecd_type&& buf, dims_type dims) -> RTNType
{
...
void sperr::CDF53::idwt3d_multi_res(std::vector<vecd_type>& h)
{
  auto dyadic = sperr::can_use_dyadic(m_dims);
...
void sperr::CDF53::m_dwt3d_wavelet_packet()
{
  /*
   *             Z
   *            /
...
void sperr::CDF53::m_idwt3d_wavelet_packet()
{
  const size_t plane_size_xy = m_dims[0] * m_dims[1];
...
void sperr::CDF53::m_dwt3d_dyadic(size_t num_xforms)
{
...
// Private Methods
//
void sperr::CDF53::m_dwt1d(itd_type array, size_t array_len, size_t num_of_lev)
{
...
void sperr::CDF53::m_idwt1d(itd_type array, size_t array_len, size_t num_of_lev)
{
...
void sperr::CDF53::m_dwt2d(itd_type plane, std::array<size_t, 2> len_xy, size_t num_of_lev)
{
  for (size_t lev = 0; lev < num_of_lev; lev++) {
...
void sperr::CDF53::m_idwt2d(itd_type plane, std::array<size_t, 2> len_xy, size_t num_of_lev)
{
  for (size_t lev = num_of_lev; lev > 0; lev--) {
...
void sperr::CDF53::m_dwt1d_one_level(itd_type array, size_t array_len)
{
  std::copy(array, array + array_len, m_qcc_buf.begin());
  if (array_len % 2 == 0) {
    this->QccWAVCDF53AnalysisSymmetricEvenEven(m_qcc_buf.data(), array_len);
    m_gather_even(m_qcc_buf.cbegin(), m_qcc_buf.cbegin() + array_len, array);
  }
  else {
    this->QccWAVCDF53AnalysisSymmetricOddEven(m_qcc_buf.data(), array_len);
    m_gather_odd(m_qcc_buf.cbegin(), m_qcc_buf.cbegin() + array_len, array);
  }
}

void sperr::CDF53::m_idwt1d_one_level(itd_type array, size_t array_len)
{
  if (array_len % 2 == 0) {
    m_scatter_even(array, array + array_len, m_qcc_buf.begin());
    this->QccWAVCDF53SynthesisSymmetricEvenEven(m_qcc_buf.data(), array_len);
  }
  else {
    m_scatter_odd(array, array + array_len, m_qcc_buf.begin());
    this->QccWAVCDF53SynthesisSymmetricOddEven(m_qcc_buf.data(), array_len);
  }
  std::copy(m_qcc_buf.cbegin(), m_qcc_buf.cbegin() + array_len, array);
}

void sperr::CDF53::m_dwt2d_one_level(itd_type plane, std::array<size_t, 2> len_xy)
{
  ...
  if (len_xy[0] % 2 == 0) {
    for (size_t i = 0; i < len_xy[1]; i++) {
      ...
      this->QccWAVCDF53AnalysisSymmetricEvenEven(m_qcc_buf.data(), len_xy[0]);
  ...
  else  // Odd length
  {
    for (size_t i = 0; i < len_xy[1]; i++) {
      auto pos = plane + i * m_dims[0];
      std::copy(pos, pos + len_xy[0], beg);
      this->QccWAVCDF53AnalysisSymmetricOddEven(m_qcc_buf.data(), len_xy[0]);
  ...
  if (len_xy[1] % 2 == 0) {
    for (size_t x = 0; x < len_xy[0]; x++) {
      ...
      this->QccWAVCDF53AnalysisSymmetricEvenEven(m_qcc_buf.data(), len_xy[1]);
  ...
  else  // Odd length
  {
    for (size_t x = 0; x < len_xy[0]; x++) {
      for (size_t y = 0; y < len_xy[1]; y++)
        m_qcc_buf[y] = *(plane + y * m_dims[0] + x);
      this->QccWAVCDF53AnalysisSymmetricOddEven(m_qcc_buf.data(), len_xy[1]);
...
void sperr::CDF53::m_idwt2d_one_level(itd_type plane, std::array<size_t, 2> len_xy)
{
  const auto max_len = std::max(len_xy[0], len_xy[1]);
  ...
  if (len_xy[1] % 2 == 0) {
    for (size_t x = 0; x < len_xy[0]; x++) {
      ...
      this->QccWAVCDF53SynthesisSymmetricEvenEven(m_qcc_buf.data() + max_len, len_xy[1]);
      for (size_t y = 0; y < len_xy[1]; y++)
        *(plane + y * m_dims[0] + x) = *(beg2 + y);
    }
  }
  else  // Odd length
  {
    for (size_t x = 0; x < len_xy[0]; x++) {
      ...
      this->QccWAVCDF53SynthesisSymmetricOddEven(m_qcc_buf.data() + max_len, len_xy[1]);
      for (size_t y = 0; y < len_xy[1]; y++)
        *(plane + y * m_dims[0] + x) = *(beg2 + y);
    }
  }
  ...
  if (len_xy[0] % 2 == 0) {
    for (size_t i = 0; i < len_xy[1]; i++) {
      ...
      this->QccWAVCDF53SynthesisSymmetricEvenEven(m_qcc_buf.data(), len_xy[0]);
  ...
  else  // Odd length
  {
    for (size_t i = 0; i < len_xy[1]; i++) {
      ...
      this->QccWAVCDF53SynthesisSymmetricOddEven(m_qcc_buf.data(), len_xy[0]);
      std::copy(beg, beg + len_xy[0], pos);
    }
  }
}

void sperr::CDF53::m_dwt3d_one_level(itd_type vol, std::array<size_t, 3> len_xyz)
{
  ...
  if (len_xyz[2] % 2 == 0) {  // Even length
    for (size_t y = 0; y < len_xyz[1]; y++) {
      for (size_t x = 0; x < len_xyz[0]; x++) {
        ...
        // Step 2
        this->QccWAVCDF53AnalysisSymmetricEvenEven(m_qcc_buf.data(), len_xyz[2]);
        // Step 3
    ...
    for (size_t y = 0; y < len_xyz[1]; y++) {
      for (size_t x = 0; x < len_xyz[0]; x++) {
        ...
        for (size_t z = 0; z < len_xyz[2]; z++)
          m_qcc_buf[z] = m_data_buf[z * plane_size_xy + xy_offset];
        // Step 2
        this->QccWAVCDF53AnalysisSymmetricOddEven(m_qcc_buf.data(), len_xyz[2]);
        // Step 3
...
void sperr::CDF53::m_idwt3d_one_level(itd_type vol, std::array<size_t, 3> len_xyz)
{
  const auto plane_size_xy = m_dims[0] * m_dims[1];
  const auto beg = m_qcc_buf.begin();  // First half of the buffer
  const auto beg2 = beg + len_xyz[2];  // Second half of the buffer
  ...
  if (len_xyz[2] % 2 == 0) {
    for (size_t y = 0; y < len_xyz[1]; y++) {
      for (size_t x = 0; x < len_xyz[0]; x++) {
        ...
        // Step 3
        this->QccWAVCDF53SynthesisSymmetricEvenEven(m_qcc_buf.data() + len_xyz[2], len_xyz[2]);
        // Step 4
        for (size_t z = 0; z < len_xyz[2]; z++)
          m_data_buf[z * plane_size_xy + xy_offset] = *(beg2 + z);
      }
    ...
    for (size_t y = 0; y < len_xyz[1]; y++) {
      for (size_t x = 0; x < len_xyz[0]; x++) {
        ...
        // Step 3
        this->QccWAVCDF53SynthesisSymmetricOddEven(m_qcc_buf.data() + len_xyz[2], len_xyz[2]);
        // Step 4
        for (size_t z = 0; z < len_xyz[2]; z++)
          m_data_buf[z * plane_size_xy + xy_offset] = *(beg2 + z);
      }
...
void sperr::CDF53::m_gather_even(citd_type begin, citd_type end, itd_type dest) const
{
  auto len = end - begin;
...
void sperr::CDF53::m_gather_odd(citd_type begin, citd_type end, itd_type dest) const
{
  auto len = end - begin;
...
void sperr::CDF53::m_scatter_even(citd_type begin, citd_type end, itd_type dest) const
{
  auto len = end - begin;
  assert(len % 2 == 0);  // This function specifically for even length input
  size_t low_count = len / 2, high_count = len / 2;
...
void sperr::CDF53::m_scatter_odd(citd_type begin, citd_type end, itd_type dest) const
{
  auto len = end - begin;
  assert(len % 2 == 1);  // This function specifically for odd length input
  size_t low_count = len / 2 + 1, high_count = len / 2;
...
auto sperr::CDF53::m_sub_slice(std::array<size_t, 2> subdims) const -> vecd_type
{
...
void sperr::CDF53::m_sub_volume(dims_type subdims, itd_type dst) const
{
...
//
void sperr::CDF53::QccWAVCDF53AnalysisSymmetricEvenEven(double* signal, size_t signal_length)
{
  size_t index;
...
void sperr::CDF53::QccWAVCDF53SynthesisSymmetricEvenEven(double* signal, size_t signal_length)
{
  size_t index;
...
void sperr::CDF53::QccWAVCDF53SynthesisSymmetricOddEven(double* signal, size_t signal_length)
{
  size_t index;
...
void sperr::CDF53::QccWAVCDF53AnalysisSymmetricOddEven(double* signal, size_t signal_length)
{
  size_t index;
