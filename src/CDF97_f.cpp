...
// Private Methods
//
void sperr::CDF97_F::m_dwt1d(itf_type array, size_t array_len, size_t num_of_lev)
{
...
void sperr::CDF97_F::m_idwt1d(itf_type array, size_t array_len, size_t num_of_lev)
{
...
void sperr::CDF97_F::m_dwt2d(itf_type plane, std::array<size_t, 2> len_xy, size_t num_of_lev)
{
  for (size_t lev = 0; lev < num_of_lev; lev++) {
...
void sperr::CDF97_F::m_idwt2d(itf_type plane, std::array<size_t, 2> len_xy, size_t num_of_lev)
{
  for (size_t lev = num_of_lev; lev > 0; lev--) {
...
void sperr::CDF97_F::m_idwt1d_one_level(itf_type array, size_t array_len)
{
  if (array_len % 2 == 0) {
...
void sperr::CDF97_F::m_idwt2d_one_level(itf_type plane, std::array<size_t, 2> len_xy)
{
  const auto max_len = std::max(len_xy[0], len_xy[1]);
...
void sperr::CDF97_F::m_dwt3d_one_level(itf_type vol, std::array<size_t, 3> len_xyz)
{
...
void sperr::CDF97_F::m_idwt3d_one_level(itf_type vol, std::array<size_t, 3> len_xyz)
{
  const auto plane_size_xy = m_dims[0] * m_dims[1];
  const auto beg = m_qcc_buf.begin();  // First half of the buffer
  const auto beg2 = beg + len_xyz[2];  // Second half of the buffer
...
void sperr::CDF97_F::m_sub_volume(dims_type subdims, itf_type dst) const
{
