...
class HuffmanEncoder : public concepts::EncoderInterface<T> {
   public:
    ...
    // SPERR port (P3): dense overload for callers who already know the
    // alphabet bounds and the frequency array. Requires:
    //   - bins are non-negative
    //   - frequencies[k] == count of bins equal to k, for k in [0, stateNum)
    //   - offset = 0 (implicit)
    // Bypasses SZo's hard-coded `ui16_range = 65536` scan in init(), which
    // buffer-overflows on alphabets > 65536 (Uf48 at vre=1e-3 has alphabet
    // ~73K) and also re-scans the input to find min/max needlessly.
    void preprocess_encode_dense(size_t num_bin, size_t *frequencies, int stateNum) {
        nodeCount = 0;
        if (num_bin == 0) {
            throw std::invalid_argument("Huffman bins should not be empty");
        }
        ...
        for (int i = 0; i < stateNum; i++) {
            if (frequencies[i] != 0) {
                qinsert(new_node(frequencies[i], i, nullptr, nullptr));
            }
        }
        ...
        buildCanonicalCode(frequencies, num_bin);
        for (unsigned int i = 0; i < huffmanTree->stateNum; i++)
            if (huffmanTree->cout[i] != 0) nodeCount++;
        nodeCount = nodeCount * 2 - 1;
    }
  ...
  //  int canonMinLen = 0;
   ...
   // bool canonReady = false;
    ...
    void buildCanonicalCode(size_t* frequencies, size_t length) {
        ...
        // SPERR port (P2): encoder packs canonical codes into uint64_t (one per
        // symbol). Anything beyond 64 bits would silently truncate.
        assert(maxLen <= 64 && "Huffman code length exceeds 64 bits");
...
*/  
    ...
    void buildCanonicalFromLengths() {
      ...
      void rebuildTreeFromCodeLengthsLSB() {
        ...
        // SPERR port (P2): decoder mirrors encoder's 64-bit code assumption.
        assert(maxLen <= 64 && "Huffman code length exceeds 64 bits");
